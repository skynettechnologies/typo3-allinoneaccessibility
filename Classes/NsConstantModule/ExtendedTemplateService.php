...
class ExtendedTemplateService extends TemplateService
{
    ...
    public function ext_getTypeData($type)
    {
        ...
        if (!$type) {
            ...
            $types = ['int' => 1, 'options' => 1, 'file' => 1, 'string' => 1, 'boolean' => 1, 'offset' => 1, 'user' => 1, 'checkbox' => 1, 'radio' => 1];
                                ...
                                foreach ($typeDat['params'] as $val) {
                                    ...
                                    $p_field .= '
                                        <div class="custom-control custom-radio">
                                            <input type="radio" id="' . $idName . $i . '" name="' . $fN . '[]" class="custom-control-input" value="' . htmlspecialchars($val) . '" ' . $checked . ' />
                                            <label class="custom-control-label" for="' . $idName . $i . '">' . $this->getLanguageService()->sL($label) . '</label>
                                        </div>';
                                    $i += 1;
                                }
